...
public extension FileManager {
    ...
    /// 获取Home的完整路径名
    /// - Returns: Home的完整路径名
    static func jq_homeDirectory() -> String {
        //获取程序的Home目录
    ...
    static func jq_DocumnetsDirectory() -> String {
    ...
    static func jq_LibraryDirectory() -> String {
        //获取程序的documentPaths目录
        //Library目录－方法1
    ...
    static func jq_CachesDirectory() -> String {
    ...
    static func jq_PreferencesDirectory() -> String {
    ...
    /// - Returns: Tmp的完整路径名
    static func jq_TmpDirectory() -> String {
        //方法1
...
public extension FileManager {
    ...
    // MARK: 移动或者拷贝的类型
    /// 移动或者拷贝的类型
    enum JQ_MoveOrCopyType {
        case file
        case directory
    }
    /// 文件管理器
    static var jq_fileManager: FileManager {
    ...
    @discardableResult
    static func jq_createFolder(folderPath: String) -> (isSuccess: Bool, error: String) {
        if !jq_judgeFileOrFolderExists(filePath: folderPath) {
            return (true, "")
        }
        // 不存在的路径才会创建
        do {
            // withIntermediateDirectories为ture表示路径中间如果有不存在的文件夹都会创建
            try jq_fileManager.createDirectory(atPath: folderPath, withIntermediateDirectories: true, attributes: nil)
            return (true, "")
        } catch _ {
            return (false, "创建失败")
        }
    ...
    @discardableResult
    static func jq_removefolder(folderPath: String) -> (isSuccess: Bool, error: String) {
        let filePath = "\(folderPath)"
        guard jq_judgeFileOrFolderExists(filePath: filePath) else {
            // 不存在就不做什么操作了
            return (true, "")
        }
        // 文件存在进行删除
        do {
            try jq_fileManager.removeItem(atPath: filePath)
            return (true, "")
        } catch _ {
            return (false, "删除失败")
        }
    ...
    @discardableResult
    static func jq_createFile(filePath: String) -> (isSuccess: Bool, error: String) {
        guard jq_judgeFileOrFolderExists(filePath: filePath) else {
            // 不存在的文件路径才会创建
            // withIntermediateDirectories 为 ture 表示路径中间如果有不存在的文件夹都会创建
            let createSuccess = jq_fileManager.createFile(atPath: filePath, contents: nil, attributes: nil)
    ...
    /// - Parameter filePath: 文件路径
    @discardableResult
    static func jq_removefile(filePath: String) -> (isSuccess: Bool, error: String) {
        guard jq_judgeFileOrFolderExists(filePath: filePath) else {
            // 不存在的文件路径就不需要要移除
            return (true, "")
        }
    ...
    /// - Returns: 文件内容
    @discardableResult
    static func jq_readfile(filePath: String) -> String? {
        guard jq_judgeFileOrFolderExists(filePath: filePath) else {
            // 不存在的文件路径就不需要要移除
            return nil
        }
        let data = jq_fileManager.contents(atPath: filePath)
    ...
    /// - Returns: 写入的结果
    @discardableResult
    static func jq_writeToFile(writeType: JQ_FileWriteType, content: Any, writePath: String) -> (isSuccess: Bool, error: String) {
        guard jq_judgeFileOrFolderExists(filePath: writePath) else {
            // 不存在的文件路径
    ...
    /// - Returns: 返回读取的内容
    @discardableResult
    static func jq_readFromFile(readType: JQ_FileWriteType, readPath: String) -> (isSuccess: Bool, content: Any?, error: String) {
        guard jq_judgeFileOrFolderExists(filePath: readPath),  let readHandler =  FileHandle(forReadingAtPath: readPath) else {
            // 不存在的文件路径
        ...
        switch readType {
            ...
            case .ArrayType:
                ...
                return (true, String.jq_toArray(readString), "")
            case .DictionaryType:
                guard let readString = String(data: data, encoding: String.Encoding.utf8) else {
                    return (false, nil, "读取内容失败")
                }
    ...
    /// - Returns: 拷贝的结果
    @discardableResult
    static func jq_copyFile(type: JQ_MoveOrCopyType, fromeFilePath: String, toFilePath: String, isOverwrite: Bool = true) -> (isSuccess: Bool, error: String) {
        // 1、先判断被拷贝路径是否存在
        guard jq_judgeFileOrFolderExists(filePath: fromeFilePath) else {
            return (false, "被拷贝的(文件夹/文件)路径不存在")
        }
        // 2、判断拷贝后的文件路径的前一个文件夹路径是否存在，不存在就进行创建
        let toFileFolderPath = jq_directoryAtPath(path: toFilePath)
        if !jq_judgeFileOrFolderExists(filePath: toFileFolderPath), type == .file ? !jq_createFile(filePath: toFilePath).isSuccess : !jq_createFolder(folderPath: toFileFolderPath).isSuccess {
            return (false, "拷贝后路径前一个文件夹不存在")
        }
        // 3、如果被拷贝的(文件夹/文件)已存在，先删除，否则拷贝不了
        if isOverwrite, jq_judgeFileOrFolderExists(filePath: toFilePath) {
            do {
                try jq_fileManager.removeItem(atPath: toFilePath)
            } catch _ {
                return (false, "拷贝失败")
            }
        }
        // 4、拷贝(文件夹/文件)
        do {
            try jq_fileManager.copyItem(atPath: fromeFilePath, toPath: toFilePath)
        } catch _ {
            return (false, "拷贝失败")
        }
        return (true, "success")
    ...
    ///   - toFile: 移动后的文件路径
    @discardableResult
    static func jq_moveFile(type: JQ_MoveOrCopyType, fromeFilePath: String, toFilePath: String, isOverwrite: Bool = true) -> (isSuccess: Bool, error: String) {
        // 1、先判断被拷贝路径是否存在
        guard jq_judgeFileOrFolderExists(filePath: fromeFilePath) else {
        ...
        // 4、移动(文件夹/文件)
        do {
            try jq_fileManager.moveItem(atPath: fromeFilePath, toPath: toFilePath)
        } catch _ {
            return (false, "移动失败")
        }
        return (true, "success")
    ...
    /** 判断文件或文件夹是否存在*/
    static func jq_judgeFileOrFolderExists(filePath: String) -> Bool {
        let exist = jq_fileManager.fileExists(atPath: filePath)
    ...
    static func jq_directoryAtPath(path: String) -> String {
        return (path as NSString).deletingLastPathComponent
    }

    // MARK: 2.12、判断目录是否可读
    static func jq_judegeIsReadableFile(path: String) -> Bool {
        return jq_fileManager.isReadableFile(atPath: path)
    }

    // MARK: 2.13、判断目录是否可写
    static func jq_judegeIsWritableFile(path: String) -> Bool {
        return jq_fileManager.isReadableFile(atPath: path)
    }
    ...
    /// - Parameter path: 文件路径
    /// - Returns: 文件扩展类型
    static func jq_fileSuffixAtPath(path: String) -> String {
    ...
    ///   - suffix: 是否需要后缀，默认需要
    /// - Returns: 文件名称
    static func jq_fileName(path: String, suffix: Bool = true) -> String {
    ...
    static func jq_shallowSearchAllFiles(folderPath: String) -> Array<String>? {
        do {
            let contentsOfDirectoryArray = try jq_fileManager.contentsOfDirectory(atPath: folderPath)
    ...
    static func jq_getAllFileNames(folderPath: String) -> Array<String>? {
        // 查看文件夹是否存在，如果存在就直接读取，不存在就直接反空
        if (jq_judgeFileOrFolderExists(filePath: folderPath)) {
            guard let subPaths = jq_fileManager.subpaths(atPath: folderPath) else {
                return nil
            }
            return subPaths
        } else {
    ...
    static func jq_deepSearchAllFiles(folderPath: String) -> Array<Any>? {
        // 查看文件夹是否存在，如果存在就直接读取，不存在就直接反空
        if (jq_judgeFileOrFolderExists(filePath: folderPath)) {
            guard let contentsOfPathArray = jq_fileManager.enumerator(atPath: folderPath) else {
                return nil
            }
    ...
    /// - Returns: 单个文件或文件夹的大小
    static func jq_fileOrDirectorySingleSize(filePath: String) -> UInt64 {
        // 1、先判断文件路径是否存在
        guard jq_judgeFileOrFolderExists(filePath: filePath) else {
            return 0
        }
        // 2、读取文件大小
        do {
            let fileAttributes = try jq_fileManager.attributesOfItem(atPath: filePath)
    ...
    /// - Parameter path: (文件夹/文件) 的路径
    /// - Returns: (文件夹/文件) 的大小
    static func jq_fileOrDirectorySize(path: String) -> String {
        if path.count == 0, !jq_fileManager.fileExists(atPath: path) {
            return "0MB"
        }
        // (文件夹/文件) 的实际大小
        var fileSize: UInt64 = 0
        do {
            let files = try jq_fileManager.contentsOfDirectory(atPath: path)
            for file in files {
                let path = path + "/\(file)"
                fileSize = fileSize + jq_fileOrDirectorySingleSize(filePath: path)
            }
        } catch {
            fileSize = fileSize + jq_fileOrDirectorySingleSize(filePath: path)
        }
    ...
    /// - Returns: (文件夹/文件)属性集合
    @discardableResult
    static func jq_fileAttributes(path: String) -> ([FileAttributeKey : Any]?) {
        do {
            let attributes = try jq_fileManager.attributesOfItem(atPath: path)
            /*
...
extension FileManager {
    ...
    /// - Returns: 转换后的文件大小
    fileprivate static func jq_covertUInt64ToString(with size: UInt64) -> String {
